...
if st.button("🔍 Predict Credit Risk"):
    try:
        ...
        # Handle multi-class: shap_values_raw is array of shape (n_samples, n_features, n_classes)
        # or a list of arrays per class
        if isinstance(shap_values_raw, list):
            # List of arrays: one per class — pick class 1 (High Risk), first sample
            ...
            # Shape (n_samples, n_features, n_classes) — pick first sample, class 1
            sv = shap_values_raw[0, :, 1]
            expected_val = explainer.expected_value[1] if hasattr(explainer.expected_value, '__len__') else explainer.expected_value
        elif isinstance(shap_values_raw, np.ndarray) and shap_values_raw.ndim == 2:
            # Shape (n_samples, n_features) — pick first sample
